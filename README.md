...
## Workflow
1. Clean fake news data
2. Train classifier (dEFEND, RoBERTa, or TextCNN) on fake news dataset
...
## Repo Locations
> Each of these repos contain an env.yml file that allows for dependency installation using anaconda. When running code in these repos remember that you must activate their respective conda environment. Some of these repos use same conda enviroment as the ReST_Adversary (another work) which is noted in their README files. For adversarial training (workflow step 5) to work properly, this repo and all the other repos need to be cloned into the same root folder and remain in separate folders.

Fake news data with cleaning script - https://github.com/ChandlerU11/fake_news_data
